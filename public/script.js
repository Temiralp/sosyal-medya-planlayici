...
const accountGroups = {
    avm: [
        'Özdilek İzmir', 'Özdilek Eskişehir', 'Özdilek Afyon', 'Özdilek Yalova', 
        'Özdilek Kocaeli', 'Özdilek Bolu', 'Özdilek Nazım', 'Özdilek Uşak', 
        'Özdilek Bursa', 'Özdilek Düzce', 'Özdilek Esenyurt', 'Özdilek ATM', 
        'Özdilek Restaurant', 'Özdilek Vakfı'
    ],
    park: [
        'Özdilek Park Bursa', 'Özdilek Park Antalya', 'Özdilek Park İstanbul', 
        'Özdilek Park M Geçit'
    ],
    konsept: [
        'Özdilek Evtekstil', 'Özdilekteyim', 'Özdilek Fırsatları'
    ],
    diger: [
        'Orange İnşaat', 'River Plaza', 'Özin Sigorta', 'Tabiat Tarım', 
        'My Auto', 'Özdilek Lokum', 'Cotonuak Clouds'
    ]
};
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const requiredElements = ['avmAccounts', 'parkAccounts', 'konceptAccounts', 'digerAccounts'];
...
function selectAll() {
    ...
    selectedAccounts = [];
    Object.values(accountGroups).flat().forEach(account => {
        platforms.forEach(platform => {
...
async function handleFormSubmit(event) {
    ...
    console.log('Form verileri:', { content, scheduledDate, scheduledTime, selectedAccounts: selectedAccounts.length });
...
async function updateProgressCount(postId) {
    try {
        ...
        if (post) {
            const completedCount = post.completedAccounts ? post.completedAccounts.length : 0;
...
function renderPostsTable(posts) {
    ...
    if (posts.length === 0) {
        ...
        tr.innerHTML = '<td colspan="8" style="text-align: center; padding: 30px; color: #666;">Henüz paylaşım yok</td>';
        tbody.appendChild(tr);
        return;
    }
    
    posts.sort((a, b) => new Date(`${a.scheduledDate} ${a.scheduledTime}`) - new Date(`${b.scheduledDate} ${b.scheduledTime}`));
    ...
    posts.forEach(post => {
        ...
        tr.innerHTML = `
            <td>
                <span class="toggle-arrow" onclick="toggleRow(${post.id})">▶</span>
            </td>
            ...
            <td>
                ${post.fileName ? 
                    `<a href="/uploads/${post.fileName}" target="_blank" class="file-link">📎 ${post.originalName || post.fileName}</a>` : 
                    '<span style="color: #999;">-</span>'
                }
            </td>
            <td class="progress-text">${completedCount}/${totalCount}</td>
            <td>
                <select class="status-select status-${post.status}" onchange="updateStatus(${post.id}, this.value)">
                    <option value="planlandı" ${post.status === 'planlandı' ? 'selected' : ''}>📅 Planlandı</option>
                    <option value="yapıldı" ${post.status === 'yapıldı' ? 'selected' : ''}>✅ Yapıldı</option>
                    <option value="beklemede" ${post.status === 'beklemede' ? 'selected' : ''}>⏳ Beklemede</option>
                    <option value="iptal" ${post.status === 'iptal' ? 'selected' : ''}>❌ İptal</option>
                </select>
            </td>
            <td>${post.createdAt ? post.createdAt : '-'}</td>
            <td>
                <button class="btn btn-danger btn-sm" onclick="deletePost(${post.id})">🗑️ Sil</button>
            </td>
        `;
        tbody.appendChild(tr);
        ...
        post.selectedAccounts.forEach(accountKey => {
            ...
            if (platform === 'Instagram' || platform === 'Facebook') {
                groupedAccounts['Instagram/Facebook'].push({accountKey, accountName, platform});
        ...
        Object.keys(groupedAccounts).forEach(groupName => {
            if (groupedAccounts[groupName].length > 0) {
                ...
                groupedAccounts[groupName].forEach(({accountKey, accountName, platform}) => {
                    const isCompleted = post.completedAccounts && post.completedAccounts.includes(accountKey);
                    accountsHtml += `
                        <div class="account-progress-item" onclick="event.stopPropagation();">
                            <input type="checkbox" 
                                   ${isCompleted ? 'checked' : ''} 
                                   onclick="event.stopPropagation();"
                                   onchange="toggleAccountComplete(${post.id}, '${accountKey}', this, event)">
