...
app.post('/api/posts', upload.single('file'), (req, res) => {
    try {
        const { content, scheduledDate, scheduledTime, selectedAccounts } = req.body;
...
app.put('/api/posts/:id/complete', (req, res) => {
    try {
        ...
        if (completed) {
            ...
            // Çıkar
            posts[postIndex].completedAccounts = posts[postIndex].completedAccounts.filter(acc => acc !== accountKey);
        }
...
app.get('/api/export', (req, res) => {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename=sosyal-medya-posts.json');
